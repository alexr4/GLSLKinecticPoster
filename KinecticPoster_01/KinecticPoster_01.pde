...
void setup(){
    ...
    poster = loadImage(datapath+"20200429-StopCovid.png");
    ...
    for(int i=0; i<nbOfLayers; i++){
        ...
        shaderList.add(loadIncludeFragment(this, datapath+"kinectic.glsl", false));
        displacementList.add(loadImage(datapath+"Displacement-"+i+".bmp"));
    }
...
void draw(){
    ...
    try{
        ...
        finalBuffer.getRamp1D(comp.getBuffer(), ramp);
    ...
    background(127);
    image(finalBuffer.getBuffer(), 0, 0, width, height);
